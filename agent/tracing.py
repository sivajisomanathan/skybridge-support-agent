...
redaction for individually-decorated node functions. See agent/config.py and
the README's LangSmith section for the full history of what was tried here,
including a real bug (raw string patterns instead of compiled regex objects
passed to create_anonymizer) that caused ALL tracing to silently fail before
...
def _redact_recursive(obj):
    ...
    string value found, regardless of key name or nesting depth. Defense-in-
    depth for individually-decorated functions -- the primary mechanism
    covering LangGraph's own automatic tracing is TRACED_CLIENT's anonymizer,
    applied via graph_tracing_context() around the graph.invoke() call.
    """
